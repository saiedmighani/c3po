...
# ✅ Fix: Define `memory_key` to allow multiple input keys
...
response_prompt = PromptTemplate(
    input_variables=["history", "query", "faiss_results"],
    template="""
        You are a helpful assistant specialized in streaming content recommendations.
        You are strictly forbidden from making up recommendations.
        Your only job is to format and return FAISS recommendations.
...
def detect_intent(user_input):
    ...
    return "movie_recommendation" if intent_label == "movie recommendation" and confidence > 0.5 else "general_conversation"
...
if __name__ == "__main__":
    ...
    while len(history) < MAX_HISTORY * 2:
        ...
        if intent == "movie_recommendation":
            ...
            if faiss_results:
                # ✅ Ensure FAISS results are passed to the LLM
                start_time = time.time()
                ...
                print(f"SearchBot: {bot_response['text']}")  # Ensure only text is printed
            ...
            bot_response = llm_chain.invoke({
                "history": "\n".join(history),
                "query": user_input,
                "faiss_results": "No content recommendation needed."
            })
            end_time = time.time()

            history.append(f"SearchBot: {bot_response['text']}")
            print(f"SearchBot: {bot_response['text']}")
            print(f"⚡ Query Latency: {(end_time - start_time) * 1000:.2f} ms\n")
